        ...
        for this_client in range(0,num_clients):
            ...
            for project_nm in os.listdir(test_dataset_dir):
                ...
                #         print(f"File '{file_path}' already exists.")
                    ...
                    with open(file_path, 'a', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        this_row=["JavaOnly", "Fed", project_nm, model_id, len(test_dataset), metric_results['BLEU'],  metric_results['METEOR'],metric_results['ROUGE']]
                        writer.writerow(this_row)
                    print("Updated for", this_client, project_nm)        
        return "Updated"
...
#for this_client in range(0,num_clients):
#    print("\n\n----------------------------------------------------------------------")
#    print("Beginning Client evaluations for client=", this_client)
#    print("EXP Start time:", datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))
#    adapter_dir = 'round1/client_'+str(this_client)+'/Ada_Impr/'
#    this_r_W_experiment(adapter_dir, this_client)
#    print("SEeeeeee!! Central experiment completed for client=", this_client)
