...
#         print(f"File '{file_path}' already exists.")
    ...
    with open(file_path, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        this_row=["JavaOnly",num_train_rows , "FedAvg", Fed_round, model_id, TRN_PARAM, LORA_TARGET_MODULES, lora_r ,before_time, after_time, num_train_epochs, len(test_dataset), metric_results['BLEU'],  metric_results['METEOR'],metric_results['ROUGE']]
...
def FedAvg(num_clients, adapter_locations, trainrows_details):
    ...
    # Initialize a list to store unique prefixes
    key_prefix = []
    # Split each key using "lora_" string and store unique prefixes
    for key in lora_adapters[0].keys():
        parts = key.split("lora_")
        if len(parts) > 1:  # Ensure "lora_" exists in the key
            prefix = parts[0]
            if prefix not in key_prefix:
                key_prefix.append(prefix)
    # LoRA weights are averaged here: We are Avg(A1,..An)and Avg(B1..Bn) instead we should Avg(A1B1,..AnBn)
    # for key in lora_adapters[0].keys():
    ...
    def Matrix_decomposition(original_matrix, B, A):
    # def Matrix_decomposition(original_matrix, m, r, n):
        #m = 4096, n = 4096, r = 64
        # # Initialize matrices B and A with random values
        # B = np.random.rand(m, r)
        # A = np.random.rand(r, n)
        # Define the learning rate and maximum number of iterations for gradient descent
        learning_rate = 0.001
        max_iterations = 5000
        # Initialize variables to track the previous error and the current iteration
        prev_error = torch.tensor(float('inf'))
        iteration = 0
        # Perform gradient descent to optimize B and A
        while iteration < max_iterations:
            # Compute the error between the reconstructed matrix and the original matrix
            error = original_matrix - B @ A
            # Compute the Frobenius norm of the error matrix
            error_norm = torch.norm(error)
            # Check if the change in the error falls below a threshold
            if torch.abs(error_norm - prev_error) < 1e-6:  # Adjust the threshold as needed
                print("We have got good decomposition, Error is really low!")
                break
            # Compute the gradients for B and A
            gradient_B = -2 * error @ torch.transpose(A, 0, 1)
            gradient_A = -2 * torch.transpose(B, 0, 1) @ error
            # Update B and A using the gradients
            B -= learning_rate * gradient_B
            A -= learning_rate * gradient_A
            # Update the previous error and iteration count
            prev_error = error_norm
            iteration += 1
        # Print the shapes of B and A
        print("Final error is: ", prev_error)
        return B, A

    #Calculate A and B as Avg(A1B1,.., AnBn)
    highest_client = normalized_trainrows.index(max(normalized_trainrows))
    print("Highest datapoints are from client: ", highest_client, "hence initialising A and B of it for final A and B respectively.")
    for key in key_prefix: #for every layer and for every projection
        keyA = key+"lora_A"+".default.weight"
        keyB = key+"lora_B"+".default.weight"
        #Now read the values, matrix multiply each AB, split A'B' as A' and B'. B (4096 * 64), A (64 * 4096)
        AvgBA = sum(
            lora_adapters[i][keyB] @ lora_adapters[i][keyA] * normalized_trainrows[i]
            for i in range(num_clients)
        )
        #Send BA, B(highest), A(highest) for init. 
        # rowA, columnA = matrix_dimensions(lora_adapters[i][keyB])
        # print("Matrix A is of dim row*col =", rowA, "*", columnA)
        # rowB, columnB = matrix_dimensions(lora_adapters[i][keyB])
        # print("Matrix B is of dim row*col =", rowB, "*", columnB)
        print("Calling matrix decomposition for key", key)
        B, A = Matrix_decomposition(AvgBA, lora_adapters[highest_client][keyB], lora_adapters[highest_client][keyA])
        #Store values to actual state dict
        avg_adapter_state_dict[keyA] = A
        avg_adapter_state_dict[keyB] = B

    print("Collation of lora and non-lora states for adapter finalisation")
    avg_loraadapter = read_adapter(adapter_locations[highest_client]) #Initialization  
...
#Create round-0's merged model if required.
...
for this_round in range(1, num_rounds+1):
    ...
    lastround_server_dir = 'round'+str(this_round -1)+'/server/'+'Ada_Impr/'
    ...
    if this_round >= 2:
        delete_directory('round'+str(this_round-2)+'/server/merged_model_Impr/')
        print(this_round-2,"server merged model deleted")
    merged_model.save_pretrained(adapter_dir+"../merged_model_Impr/",safe_serialization=True) #TBW
...
#     update_basemodel_inAda_ImprCfg(adapter_dir)